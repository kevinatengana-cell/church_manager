...
class _SortiesScreenState extends State<SortiesScreen> {
  // Use a key to force rebuild of child lists when returning from Add
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Sorties d\'évangélisation'),
          bottom: const TabBar(
            labelColor: AppTheme.primary,
            unselectedLabelColor: AppTheme.textSecondary,
            indicatorColor: AppTheme.primary,
            tabs: [
              Tab(icon: Icon(Icons.event), text: 'Mes Sorties'),
              Tab(icon: Icon(Icons.move_to_inbox), text: 'Rapports Reçus'),
            ],
          ),
        ),
        body: TabBarView(
          children: [
            _SortiesList(key: ValueKey('local_$_listKey'), imported: false),
            _SortiesList(key: ValueKey('imported_$_listKey'), imported: true),
          ],
        ),
...
class _SortiesList extends StatefulWidget {
  final bool imported;
  const _SortiesList({Key? key, required this.imported}) : super(key: key);

  @override
  State<_SortiesList> createState() => _SortiesListState();
}

class _SortiesListState extends State<_SortiesList> {
  final _db = DatabaseService();

  @override
  Widget build(BuildContext context) {
    return FutureBuilder<List<Sortie>>(
      future: _db.getSorties(imported: widget.imported),
      builder: (ctx, snap) {
        if (!snap.hasData) {
        ...
        if (sorties.isEmpty) {
          return Center(
            child: Column(
              ...
              children: [
                Icon(widget.imported ? Icons.inbox : Icons.event_busy,
                    size: 64, color: AppTheme.textSecondary.withOpacity(0.4)),
                const SizedBox(height: 16),
                Text(widget.imported ? 'Aucun rapport reçu' : 'Aucune sortie enregistrée',
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  if (sortie.isImported && sortie.senderName != null) ...[
                    const SizedBox(height: 4),
                    Row(
                      children: [
                        const Icon(Icons.move_to_inbox, size: 14, color: AppTheme.primary),
                        const SizedBox(width: 4),
                        Text('Reçu de ${sortie.senderName}',
                            style: const TextStyle(
